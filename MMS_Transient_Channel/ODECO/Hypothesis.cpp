...
// For writing the sensor locations to file
#include <fstream>
  ...
  try
    {
      ...
      // tuple(.083333333,.25,0.4166666667,0.5833333333,.75,0.9166666667)
      double y0 = .083333333;
      Sundance::setOption("y0",y0,"1 of 2 y-coordinates specified for the four sensor locations");

      double y1 = .25;
      Sundance::setOption("y1",y1,"2 of 2 y-coordinates specified for the four sensor locations");
	...
	+ "/tol" + tolFileValue.str() + "/";
      ...
      Array<double> xSensorPositions(numSens);
      Array<double> ySensorPositions(numSens);
      if(numSens==12)
	{
	  xSensorPositions = tuple(0.25,0.75);
	  ySensorPositions = tuple(.083333333,.25,0.4166666667,0.5833333333,.75,0.9166666667);
	}
      else if(numSens==4)
	{
	  xSensorPositions = tuple(0.25,0.75);
	  ySensorPositions = tuple(y0,y1);
	}
      else if(numSens==2)
	{
	  //xSensorPositions.resize(1);
	  // Two along the same vertical line
	  xSensorPositions = tuple(0.25);
	  ySensorPositions = tuple(y0,y1);

	  // Two along the same horizontal line
	  // xSensorPositions = tuple(0.25,0.75);
	  // ySensorPositions = tuple(y0);
	}
      else if(numSens==1)
	{
	  //xSensorPositions.resize(1);
	...
	{
	  cout << "Hi from backup 12" << endl;
	  numSens=12;
	  xSensorPositions = tuple(0.25,0.75);
	  ySensorPositions = tuple(.083333333,.25,0.4166666667,0.5833333333,.75,0.9166666667);
	}
      Array<Point> positionArray;
      positionArray.resize(numSens);
      // Assigns values for (x1,y1) through (x1,yN), then goes to x2 and so forth
      //cout << "Location of sensors" << endl;
      ofstream file;
      file.open("../Graphing/Hypothesis_numSens="+Teuchos::toString(numSens)+".txt");
      file << "x,y" << endl;
      for(int i=0; i<xSensorPositions.length(); i++)
	for(int j=0; j<ySensorPositions.length(); j++)
	  {
	    positionArray[j+ySensorPositions.length()*i].resize(spatialMesh.spatialDim() );
	    positionArray[j+ySensorPositions.length()*i][0] = xSensorPositions[i];
	    positionArray[j+ySensorPositions.length()*i][1] = ySensorPositions[j];
	    //cout << positionArray[j+ySensorPositions.length()*i] << endl;
	    file << positionArray[j+ySensorPositions.length()*i][0] << ","
		 << positionArray[j+ySensorPositions.length()*i][1] << endl;
	  }      
      cout << endl << endl;
      file.close();
      ...
      if(writeResults)
	{
	  ...
	  vtkDir += "tol" + tolFileValue.str() + "/tFinal" + Teuchos::toString(int(tFinal))
	    ...
	    + "nx" + Teuchos::toString(nx) + "nt" + Teuchos::toString(nSteps) + "/Hypothesis/"
